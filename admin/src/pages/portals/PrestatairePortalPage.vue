<template>
  <section class="stack">
    <header class="page-header">
      <div>
        <div class="eyebrow">Espace prestataire</div>
        <h2 class="page-title">Pilotage de l'activite</h2>
      </div>
    </header>
      ...
      <article class="surface-card section-card">
        ...
        <ul class="portal-list">
          <li v-for="item in snapshot.prestations" :key="item.id">
            ...
            <span>{{ item.type }} · {{ item.price }} EUR</span>
          </li>
        </ul>
      </article>
    </div>
...
onMounted(async () => {
  ...
  snapshot.value = data.prestataire;
});
</script>
...
.quick-card {
  ...
  background: linear-gradient(180deg, rgba(27, 138, 138, 0.08), rgba(255, 255, 255, 0.8));
  border-radius: 18px;
  padding: 16px;
  text-align: left;
  font-weight: 700;
