<template>
    <main class="public-dashboard">
        ...
        <header class="content-header">
            <div class="header-left">
                <p class="sidebar__category2">
                    ACCUEIL > CATALOGUE >
                    {{ annonce?.titre ? annonce.titre.toUpperCase() : "..." }}
                </p>
            </div>
        ...
        <div v-else-if="annonce?.id">
                        ...
                        <ul class="product-details-list">
                            <li>
                                ...
                                <span class="detail-value"
                                    ><strong>{{
                                        annonce.etat_objet || "Non renseigné"
                                    }}</strong></span
                                >
                            </li>
                            <li>
                        ...
                        >
                            ...
                            <div class="vendor-info">
                                <span class="vendor-name">{{
                                    vendeur.prenom ||
                                    "Utilisateur #" + annonce.id_vendeur
                                }}</span>
            ...
            <section
                v-if="autresAnnonces.length > 0"
                class="other-annonces-section"
            >
                <h2 class="section-title-large">
                    Autres annonces de l'utilisateur
                </h2>

                <div class="annonces-grid">
                    <article
                        v-for="ann in autresAnnonces"
                        :key="ann.id"
                        class="annonce-card"
                    >
                        <div
...
const isMyAnnonce = computed(() => {
    if (!annonce.value || !currentUserId.value) return false;
    return parseInt(annonce.value.id_vendeur) === parseInt(currentUserId.value);
});
...
const fetchFavoriStatus = async () => {
    try {
        const res = await fetch(
            `${API_URL}/annonces/${route.params.id}/favori/${currentUserId.value}`,
        );
        if (res.ok) {
        ...
        console.error("Erreur récupération favori", e);
    }
};
...
const fetchAnnoncePrincipale = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            annonce.value = data;
            if (annonce.value.id_categorie)
                fetchCategory(annonce.value.id_categorie);
            ...
            console.warn("Annonce introuvable, redirection vers le catalogue.");
            router.push("/catalogue");
        }
    } catch (e) {
        console.error("Erreur serveur/réseau :", e);
        router.push("/catalogue");
    } finally {
        loading.value = false;
    }
...
const modifierAnnonce = () => {
    router.push(`/profil/modifyAnnonce/${annonce.value.id}`);
};
...
const supprimerAnnonce = async () => {
    ...
    try {
        ...
        const res = await fetch(`${API_URL}/annonces/${annonce.value.id}`, {
            method: "DELETE",
            headers: {
                Authorization: token,
            },
