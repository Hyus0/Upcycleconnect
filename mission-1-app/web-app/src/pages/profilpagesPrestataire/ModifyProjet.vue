    ...
    <div class="page-container">
        <header class="content-header">
            <div class="header-left">
                <p class="sidebar__category2">ACCUEIL > MES PROJETS > MODIFIER</p>
                <h1 class="hero-title1">MODIFIER LE PROJET</h1>
                ...
                <div class="form-card main-info-card">
                    ...
                    <div class="form-group">
                        ...
                        <small class="char-count"
                            >{{ form.description_courte?.length || 0 }}/255</small
                        >
                    </div>
                ...
                <div class="right-column">
                    <div class="steps-global-header">
                        <div class="steps-header-text">
                            ...
                            <p class="card-subtitle">
                                Réorganisez ou modifiez votre tutoriel.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div v-if="form.etapes.length === 0" class="no-steps-msg">
                        Aucune étape. Le projet est une simple vitrine.
                    </div>
...
onMounted(async () => {
    try {
        const res = await fetch(`${API_URL}/projet/${projetId}`);
        if (res.ok) {
            const data = await res.json();
            if (data.id_createur !== currentUserId.value) {
            ...
            form.value = {
                ...
                etapes: data.etapes ? [...data.etapes] : [],
            };
        } else {
            errors.value.push("Projet introuvable.");
        }
    } catch (error) {
        console.error(error);
...
const validateFrontend = () => {
    ...
    if (!form.value.image_url)
        errors.value.push("Une image de couverture est obligatoire.");
...
const handleSubmit = async () => {
    ...
    try {
        const response = await fetch(`${API_URL}/projets/${projetId}`, {
            method: "PUT",
            headers: {
                Authorization: token,
...
.steps-global-header .card-subtitle {
    margin-bottom: 0;
}
.btn-add-step {
    background: none;
